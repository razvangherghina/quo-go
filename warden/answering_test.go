...
func exchange(t *testing.T, g *ground, caller *warden.Warden, r warden.Reach, e string) (envelope.Answer, error) {
	...
	if reply == nil {
		t.Fatal("the door met the call with silence")
	}
	return caller.Hear(reply)
}
...
func TestACargoCarriesTheHeirOrArrivesUnrotatable(t *testing.T) {
	g := stand(t)
	caller := calling(t, g, "caller")
	being, _, err := caller.Hold(&todo{}, warden.Holding{
		Blueprint: todoText,
		Keys:      warden.Keys{Secret: secret("caller/being"), HeirSecret: secret("caller/beingHeir")},
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestACallerChoosesWhichNumberItOpensWith(t *testing.T) {
	...
	r.NextHeir = &mine
	r.Seq = &opening
	message, seq, err := caller.Ask(r)
	if err != nil || seq != opening {
		t.Fatalf("the ask spent %d (%v), want %d", seq, err, opening)
	}
	reply := g.w.Arrive(message, nil)
	if reply == nil {
		t.Fatal("the door refused an opening number above one")
	}
	...
	back.Seq = &spent
	if _, _, err := caller.Ask(back); err == nil {
		t.Fatal("a number this relation had already spent went out again")
	}
	if _, next, err := caller.Ask(reach(g.inv.Warden)); err != nil || next != opening+1 {
...
func TestAnAnswerFromADoorThisAskNeverWentTo(t *testing.T) {
	...
	other := house(t, "other")
	room, _, err := other.Hold(&todo{}, warden.Holding{
		Blueprint: todoText,
		Keys:      warden.Keys{Secret: secret("other/room"), HeirSecret: secret("other/roomHeir")},
	})
	if err != nil {
		t.Fatal(err)
	}
	inv, err := other.GrantAs(room,
	...
	message, _, err := elsewhere.Ask(r)
	if err != nil {
		t.Fatal(err)
	}
	reply := other.Arrive(message, nil)
	if reply == nil {
		t.Fatal("the other door said nothing at all")
	}
	...
	if _, err := caller.Hear(reply); err == nil {
...
func TestAnAnswerNothingAwaitsIsSilence(t *testing.T) {
	...
	if n := warden.AsksOut(caller, g.inv.Warden); n != 1 {
	...
	if n := warden.AsksOut(caller, g.inv.Warden); n != 0 {
...
func TestTwoAsksWhoseAnswersCouldNotBeToldApart(t *testing.T) {
	...
	rotation := reach(g.inv.Warden)
	rotation.NextHeir = &first
	if _, seq, err := caller.Ask(rotation); err != nil || seq != 1 {
	...
	second := secret("caller/heir2")
	rotation.NextHeir = &second
	if _, _, err := caller.Ask(rotation); err == nil {
...
// TestAcceptLeavesNothingAwaiting holds the same rule where accepting an
// invitation meets it. Both of its rotate-and-asks open at one, because each
// starts the far door's mark fresh — so the first must be settled before the
// second is composed, or the second is the ask this kit refuses to send. When
// accept is done, nothing is left awaiting an answer that will never come.
func TestAcceptLeavesNothingAwaiting(t *testing.T) {
	delivery := warden.NewMemory()
	g := standDelivered(t, delivery)
	delivery.Attach("mem://door", g.w)
	g.w.Publish("mem://door")
	inv, err := g.w.GrantAs(g.being, warden.Keys{
		Secret: secret("accept/voice"), HeirSecret: secret("accept/voiceHeir"),
	}, g.w.Padlock(), g.w.Hints())
	if err != nil {
		t.Fatal(err)
	}

	caller := housed(t, "caller", delivery)
	handle, err := sole(caller.Accept(context.Background(), inv, warden.Accepting{Label: "there"}))
	if err != nil {
		t.Fatalf("accepting the invitation was refused: %v", err)
	}
	if n := warden.AsksOut(caller, inv.Warden); n != 0 {
		t.Fatalf("%d asks still await an answer after accept", n)
	}
	// And the handle it hands back is a handle: the being answers through it.
	if v, ok := handle.Call(context.Background(), "count"); !ok || v.(int64) != 0 {
		t.Fatalf("the handle answered %v %v", v, ok)
	}
	if n := warden.AsksOut(caller, inv.Warden); n != 0 {
		t.Fatalf("%d asks await an answer after a settled call", n)
	}
}
